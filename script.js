...
const btnRemoveTasks = document.querySelector('.btn-clean-tasks');
...
window.addEventListener('FocoFinalizado', () => {
    ...
    // botaoFoco.disabled = false; //Habilita o botao para reiniciar o foco
    ...
    if (activeTaskIndex !== null) {
        tasks[activeTaskIndex].completa = true; // Marca a tarefa como completa
        localStorage.setItem('tarefas', JSON.stringify(tasks));
        renderTasks(); // Re-renderiza as tarefas
        taskDescriptionDisplay.innerHTML = '<p><strong>Foco Finalizado!</strong></p>';
        botaoFoco.disabled = false;
        tempo = 0.1 * 60; // Reseta o cronômetro
    }
...
function renderTasks() {
    ...
    tasks.forEach((task, index) => {
        ...
        if(task.completa) {
            ...
            editBotao.setAttribute('disabled', 'disabled')
        }
...
btnRemoveTasks.onclick = () => removerTarefas();